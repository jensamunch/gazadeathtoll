...
export default function HomeIntl() {
  ...
  const seedImageNames = useMemo(() => [
    'slice_final_1_1.png','slice_final_1_2.png','slice_final_1_3.png',
    'slice_final_2_1.png','slice_final_2_2.png','slice_final_2_3.png',
    'slice_final_3_1.png','slice_final_3_2.png','slice_final_3_3.png',
  ], [])

  const squareImageUrlForId = useCallback((id: string): string => {
    let hash = 0
    for (let i = 0; i < id.length; i++) hash = (hash * 31 + id.charCodeAt(i)) | 0
    const idx = Math.abs(hash) % seedImageNames.length
    return `/api/seed/${seedImageNames[idx]}`
  }, [seedImageNames])
  ...
  if (!mounted) {
    return (
      <main className="p-6"><div className="py-24 flex items-center justify-center">{t('loading')}</div></main>
    )
  }
  ...
  return (
    <main className="p-6">
      ...
      <div className="my-2 text-sm text-muted-foreground">
        {t('loadedSummary', { loaded: data.length.toLocaleString(), total: total.toLocaleString() })}
      </div>
      ...
      {view === 'list' ? (
        <Table className="min-w-[1200px]">
          ...
          <TableBody>
            {loading ? (
              <TableRow><TableCell colSpan={9} className="py-10 text-center">{t('loading')}</TableCell></TableRow>
            ) : data.length === 0 ? (
              <TableRow><TableCell colSpan={9} className="py-10 text-center">{t('noResults')}</TableCell></TableRow>
            ) : (
              data.map((p) => (
                <TableRow key={p.id} className="cursor-pointer" onClick={() => setEditing(p)}>
                  ...
                  <TableCell>
                    <div className="flex items-center">
                      {hasImageForId(p.id) ? (
                        <img src={squareImageUrlForId(p.id)} alt={p.name || p.enName || p.id} className="size-8 rounded-md object-cover" />
                      ) : (
        ...
        <div>
          {loading ? (
            ...
            <div className="py-10 text-center">{t('noImages')}</div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {galleryData.map((p) => (
                <div key={p.id} className="rounded-md border overflow-hidden">
                  <img src={squareImageUrlForId(p.id)} alt={p.name || p.enName || p.id} className="w-full aspect-square object-cover" />
                  <div className="p-2">
                    ...
                    <div className="text-muted-foreground text-xs">{t('dob')} {formatDateOnly(p.dob)} • {t('dod')} {dodForId(p.id)}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => { if (!o) setEditing(null) }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('proposeEditTitle')}</DialogTitle>
          </DialogHeader>
          {editing && (
            <EditForm
              t={(key: string, values?: Record<string, any>) => t(key as any, values)}
              person={editing}
              imageUrl={hasImageForId(editing.id) ? squareImageUrlForId(editing.id) : null}
...
function EditForm({ t, person, imageUrl, onClose, onSaved }: { t: (k: string, v?: any) => string; person: Person; imageUrl?: string | null; onClose: () => void; onSaved: () => void }) {
    ...
    <div className="grid gap-3">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-sm text-muted-foreground">{t('dob')}</label>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button onClick={save} disabled={saving}>{saving ? t('proposing') : t('proposeEdit')}</Button>
      </div>
    </div>
  )
}
